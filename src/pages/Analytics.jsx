        ...
        <div className="lg:col-span-7 bg-white rounded-2xl shadow-xs border border-slate-200/80 p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-800">6 Dedicated Storage Shades Capacity</h3>
            </div>
            <span className="text-[11px] font-bold text-slate-500">Live Bin Occupancy</span>
          </div>

          <div className="space-y-3.5">
            {shadesCapacity.map((shade) => (
              <div key={shade.id} className="p-3.5 rounded-xl border border-slate-200/70 hover:border-slate-300 transition bg-slate-50/50">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-800 truncate">{shade.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${shade.lightColor}`}>
                      {shade.status}
                    </span>
                    <span className="font-mono text-xs font-black text-slate-900">{shade.fillPercent}%</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mt-2 text-[11px] text-slate-500">
                  <span>Occupied: <strong className="text-slate-800">{shade.occupied.toLocaleString()}</strong> {shade.unit}</span>
                  <span>Capacity: <strong className="text-slate-800">{shade.totalCapacity.toLocaleString()}</strong> {shade.unit}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-xs border border-slate-200/80 p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-800">Inventory Movement Velocity (Fast vs Slow Moving SKUs)</h3>
          </div>
          <Link to="/reports" className="text-xs font-bold text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
